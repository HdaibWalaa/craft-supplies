...
export type CartWithItems = Omit<ApiCart, "items"> & {
  items: Array<{
    ...
    productId: string;
    variantId: string;
    product: { id: string; name: string; slug: string; images: string; thumbnail?: string };
...
export async function getCart(): Promise<CartWithItems | null> {
  ...
  return {
    ...
    items: cart.items.map((item) => ({
      ...
      product: { id: item.product.id, name: item.product.name, slug: item.product.slug, images: JSON.stringify(item.product.images), thumbnail: getProductPreviewImage(item.product)?.url ?? "" },
